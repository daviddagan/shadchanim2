import 'package:flutter_app/screens/moreMeArea.dart';
import 'package:flutter_app/widgets/DateText.dart';
...
class MoreMeBirthday extends StatefulWidget {
  @override
  _MoreMeBirthdayState createState() => _MoreMeBirthdayState();
}
...
class _MoreMeBirthdayState extends State<MoreMeBirthday> {
  bool badDate = false;
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            GradientButton.getGradiantButton(context, addUserMoreMe, 'CONTINUE', Colors.black, 20, dateVarified),
          ],
        ),
      ),
    );
  ...
  void validateDate() {
    ...
    if (d1 != "" && d2 != "" && m1 != "" && m2 != "" && y1 != "" && y2 != "" && y3 != "" && y4 != "") {
      int day = int.parse(d1) * 10 + int.parse(d2);
      int month = int.parse(m1) * 10 + int.parse(m2);
      int year = int.parse(y1) * 1000 + int.parse(y2) * 100 + int.parse(y3) * 10 + int.parse(y4);
      DateTime date = new DateTime(year, month, day);
      if (date.year != year || date.month != month || date.day != day || date.isAfter(DateTime.now())) {
        setState(() {
          dateVarified = false;
          badDate = true;
  ...
  void addUserMoreMe() {
    Navigator.of(context2).push(new MaterialPageRoute(builder: (context) => new MoreMeArea()));
  }
}
